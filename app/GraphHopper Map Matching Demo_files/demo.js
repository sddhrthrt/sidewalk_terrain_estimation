            ...
            mmClient.doRequest(content, function (json) {
                if (json.message) {
                    ...
                    $.ajax({
                      ...
                      data: JSON.stringify({"points": json.paths[0].points.coordinates,
                                            "filename": file.name}),
                      success: function(d, s, x){
                        for(var i=1; i<matchedPath.points.coordinates.length; i++){
                          if(d[i]){
                            var geojsonFeature = {
                              type: "Feature",
                              geometry: {
                                coordinates: [matchedPath.points.coordinates[i-1], matchedPath.points.coordinates[i]],
                                type: "LineString"
                              },
                              properties: {style: {color: "#cc3300", weight: 6, opacity: 0.6}}
                            };
